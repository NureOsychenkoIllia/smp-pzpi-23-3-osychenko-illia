...
function setProfile(&$userProfile)
{
    if (isset($_POST['profileSubmit'])) {
        ...
        if (strlen($name) == 0 || !preg_match('/\p{L}/u', $name)) {
            $errors[] = "Імʼя користувача не може бути порожнім і повинно містити хоча б одну літеру.";
        }
        
        if ($age < 7 || $age > 150) {
            $errors[] = "Користувач не може бути молодшим 7-ми або старшим 150-ти років.";
        }
...
function showProducts($products)
    {
        ...
        for ($i = 0; $i < count($keys); $i++) {
            ...
            echo "<form method='POST' action='index.php?action=add' class='add-to-cart-form'>";
